...
use Response,Validator,Input;
use Illuminate\Http\Request;
use DB,App\User,App\Student,App\Communication,App\CommunicationStudent,App\MailQueue;
use App\Lead;
...
class CommunicationController extends Controller {
	...
	public function init(Request $request){
		...
		$only_active = (Input::get("only_active") == 1) ? true : false;
	...
	public function getStudents(Request $request){
		...
		$removed_students = Input::get('removed_students');
		...
		$students = Student::select("students.id",'students.name','students.father_mob','students.mother_mob','center.center_name','students.dob','students.doe')->join("groups","groups.id",'=','students.group_id')->join('center','center.id','=','groups.center_id')->whereNotIn('students.id',$excluded_student_ids);
		...
		if(Input::has("date_start")){
			if(Input::get("date_start")){
				$date_str = strtotime(Input::get("date_start"));
		...
		if(Input::has("date_end")){
			if(Input::get("date_end")){
				$date_str = strtotime(Input::get("date_end"));
				$students = $students->where("students.dob","<=",$date_str);
			}
		}

		if(Input::has('paused')){
			if(Input::get("paused") == 1){
				$students = $students->where("paused",1);
			} elseif(Input::get("paused") == 2){
				$students->where("paused",0);
			}
		}

		if(Input::has("min_renew_days")){
			if(Input::get("min_renew_days") != ""){
				$date_ref = strtotime(Input::get("min_renew_days"));
				$students = $students->where("students.doe",">=",$date_ref);
			}
		}

		if(Input::has("max_renew_days")){
			if(Input::get("max_renew_days") != ""){
				$date_ref = strtotime(Input::get("max_renew_days"));
				$students = $students->where("students.doe","<=",$date_ref);
			}
		}

		if(Input::has("downloaded_app")){
			if(Input::get("downloaded_app") == 1){
				$students = $students->join("webserver_studentcustomermapping","webserver_studentcustomermapping.student_id","=","students.id");
			} else if(Input::get("downloaded_app") == 2){
		...
		if(Input::has("mobile")){
			if(Input::get("mobile") != ""){
				$mobiles = explode(',', Input::get("mobile"));
				$mobile_numbers = [];
	...
	{	
		...
		if(in_array(1,$types)){
			...
			$template = DB::table("sms_templates")->where('id',Input::get("template_id"))->first();
			$sms_content = '';
			if($template){

				$sms_content = $template->template;
				$sms_content = str_replace('{#var1#}',Input::get('variable1'), $sms_content);
				$sms_content = str_replace('{#var2#}',Input::get('variable2'), $sms_content);
				$sms_content = str_replace('{#var3#}',Input::get('variable3'), $sms_content);
			}
		}

		if(Input::get('demo_check')){
			$data['success'] = true;

			$demo_mobile = Input::get('demo_mobile');
			$demo_email = Input::get('demo_email');

			if($demo_mobile && $template){
				// Lead::sendSMS($demo_mobile , Input::get('sms_content'),Input::get('sms_type'));
				Lead::sendSMS($demo_mobile , $sms_content, Input::get('sms_type'), $template->dlt_template_id, $template->dlt_sender_id);
			}

			if($demo_email){
				Lead::sendEmail($demo_email, Input::get('subject'), Input::get("content"));
			}
			...
			if(sizeof($types) == 0){
				...
				$com->sms_type = Input::get('sms_type');
				$com->message = Input::get('content');
				$com->subject = Input::get('subject');
				$com->source_type = 1;
				$com->added_by = $user->id;


				if(in_array(1,$types)){
					$com->template_id = Input::get("template_id");
					$com->variable1 = Input::get("variable1");
					$com->variable2 = Input::get("variable2");
					$com->variable3 = Input::get("variable3");
				...
				$student_ids = Input::get('student_ids');
				$removed_students = Input::get('removed_students');
				...
				foreach ($students as $student) {
					...
					if(in_array(1, $types)){
						if($student->mobile){
							MailQueue::createSMS($student->mobile, $sms_content,Input::get('sms_type'),Input::get("template_id"));
							$mobiles[] = $student->mobile;
						}

						if($student->father_mob && !in_array($student->father_mob,$mobiles)){
							MailQueue::createSMS($student->father_mob, $sms_content,Input::get('sms_type'),Input::get("template_id"));
							$mobiles[] = $student->father_mob;
						}

						if($student->mother_mob && !in_array($student->mother_mob,$mobiles)){
							MailQueue::createSMS($student->mother_mob, $sms_content,Input::get('sms_type'),Input::get("template_id"));
						}
					}

					if(in_array(2, $types)){

						$message = Lead::getContent(Input::get('content'));

						if($student->email){
							MailQueue::createEmail($student->email,Input::get('subject'),  $message);
							$emails[] = $student->email;
						}
						if($student->father_email && !in_array($student->father_email,$emails)){
							MailQueue::createEmail($student->father_email,Input::get('subject'),  $message);
							$emails[] = $student->father_email;
						}
						if($student->mother_email && !in_array($student->mother_email,$emails)){
							MailQueue::createEmail($student->mother_email,Input::get('subject'),  $message);
						}
					}
				}
	...
	public function comm_students(Request $request){
		...
		$student_ids = DB::table('communication_students')->where('communication_id',Input::get('id'))->pluck('student_id')->toArray();
